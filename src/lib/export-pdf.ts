...
function generateFeedbackList(data: ApiResults) {
  ...
  if ((data.relevance ?? 0) >= 0.7)
    strengths.push("Answer was on-topic and addressed the question well.");
  else weaknesses.push("Answer could be more focused on the question asked.");

  if ((data.sentiment ?? 0) >= 0.65)
    strengths.push("Positive and confident tone in your response.");
  else {
    weaknesses.push("Tone came across as hesitant or neutral.");
    suggestions.push("Practice speaking with more enthusiasm and conviction.");
  }

  if (data.confidence_label === "High")
    strengths.push("Strong vocal confidence detected across the interview.");
  else if (data.confidence_label === "Medium")
    suggestions.push("Build on your confidence — try rehearsing aloud before interviews.");
  else
    weaknesses.push(
