    ...
    <div className="relative min-h-screen text-foreground px-6 py-24 md:py-32 font-mono">
      ...
      <div className="relative z-10 max-w-5xl mx-auto space-y-8">
        ...
        <section className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
          ...
          {dbLoading ? (
            <div className="flex items-center gap-3 text-white/50">
              <Loader2 className="w-4 h-4 animate-spin" /> Pinging cluster...
            </div>
          ) : dbHealth ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                ...
                <div
                  className={`text-base font-semibold ${dbHealth.status === "Healthy" ? "text-green-400" : "text-red-400"}`}
                >
                  {dbHealth.status}
                </div>
              </div>
        ...
        <section className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
          ...
          {modelsLoading ? (
            <div className="flex items-center gap-3 text-white/50">
              <Loader2 className="w-4 h-4 animate-spin" /> Querying memory states...
            </div>
          ) : modelsHealth ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {["nlp_processor", "vision_processor", "confidence_predictor", "fusion_scorer"].map(
                (modelName) => {
                    ...
                    >
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-xs text-white/40">
                          Weights Loaded: {info.weights_loaded ? "Yes" : "No"}
                        </span>
                      </div>
                      <div
                        className={`px-3 py-1 rounded-full text-xs font-semibold border ${isWarm ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-white/5 text-white/40 border-white/10"}`}
                      >
                        {info.status}
                      </div>
                    </div>
