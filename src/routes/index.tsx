...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Multimodal AI-powered interview analysis using Speech, Vision, and NLP.",
      },
    ],
  }),
  component: Landing,
...
const features = [
  {
    ...
    desc: "Whisper transcription, MFCC, pitch, energy, speech rate.",
  },
  {
    icon: Eye,
    title: "Emotion Detection",
    desc: "CNN trained on FER-2013 for real-time facial affect.",
  },
  {
    icon: MessageSquareText,
    title: "NLP Understanding",
    desc: "DistilBERT sentiment + semantic relevance scoring.",
  },
  {
    icon: Network,
    title: "Neural Fusion",
    desc: "ANN fusion layer combines all modalities into one signal.",
  },
];
    ...
    <div className="relative min-h-screen text-foreground" ref={containerRef}>
      ...
      <section className="relative w-full min-h-screen flex flex-col overflow-hidden bg-transparent pt-32 pb-10">
        ...
        >
          ...
          >
            Create fully functional, AI-optimized evaluations in seconds
          ...
          >
            {!isAuthenticated ? (
              <>
                ...
                >
                  Sign Up{" "}
                  <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" />
                </Link>
              </>
            ) : (
              <>
          ...
          >
            ...
            <div className="flex flex-col sm:grid sm:grid-cols-2 md:flex md:flex-row w-full justify-between items-start md:items-center px-2 md:px-4 gap-6 md:gap-0 md:divide-x md:divide-white/10 z-10">
              ...
              >
                <Brain
                  className="w-8 h-8 text-[#4a6fff] flex-shrink-0 drop-shadow-md"
                  strokeWidth={1.5}
                />
                <div className="flex flex-col text-left">
                  <span className="text-[15px] font-semibold text-white drop-shadow-sm whitespace-nowrap">
                    Multimodal AI
                  </span>
                  <span className="text-[13px] text-white/70 drop-shadow-sm whitespace-nowrap">
                    Speech, Vision, NLP Fusion
                  </span>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="flex flex-col text-left min-w-0">
                  <span className="text-[15px] font-semibold text-white drop-shadow-sm truncate">
                    Enterprise Ready
                  </span>
                  <span className="text-[13px] text-white/70 drop-shadow-sm truncate">
                    Secure & Scalable
                  </span>
                </div>
              </motion.div>
            ...
            <div className="grid gap-6 grid-cols-2 md:grid-cols-4">
              {stats.map((s, i) => (
                ...
                >
                  ...
                  <div className="text-sm font-medium text-white/50 group-hover:text-white/70 transition-colors uppercase tracking-widest relative z-10">
                    {s.label}
                  </div>
                  <div className="text-4xl font-semibold tracking-tighter text-white relative z-10">
                    {s.value}
                  </div>
                </motion.div>
              ))}
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              {features.map((f, i) => (
                ...
                >
                  ...
                  <f.icon
                    className="h-8 w-8 text-[var(--brand-blue)] group-hover:text-white mb-6 relative z-10 group-hover:scale-110 transition-transform duration-300"
                    strokeWidth={1.5}
                  />
                  <h3 className="text-lg font-medium text-white/90 group-hover:text-white relative z-10 transition-colors">
                    {f.title}
                  </h3>
                  <p className="mt-2 text-sm text-white/50 group-hover:text-white/70 relative z-10 transition-colors">
                    {f.desc}
                  </p>
                </motion.div>
              ))}
        ...
        <section className="relative py-12 px-8 md:px-28 z-20 overflow-hidden">
          <div className="max-w-[1200px] mx-auto">
            <div className="mb-20 text-center flex flex-col items-center">
              <span className="text-sm font-medium text-[var(--brand-blue)] uppercase tracking-widest bg-[var(--brand-blue)]/10 px-3 py-1 rounded-full border border-[var(--brand-blue)]/20">
                How it works
              </span>
              <h2
                className="mt-6 text-4xl md:text-5xl font-semibold tracking-tighter"
                style={{ fontFamily: "var(--font-serif)" }}
              >
                From Interview to Insight.
              </h2>
              <p className="mt-4 text-muted-foreground max-w-xl text-sm">
                Watch as raw multimodal data flows through our neural architecture to produce
                perfectly objective evaluations.
              </p>
            </div>
                ...
                {[
                  ...
                  >
                    ...
                    <div className="relative w-14 h-14 rounded-full bg-card/40 backdrop-blur-xl border border-white/5 flex items-center justify-center shrink-0 group-hover:border-[var(--brand-blue)]/40 group-hover:bg-[var(--brand-blue)]/10 transition-all duration-300 shadow-xl shadow-black/50 group-hover:shadow-[0_0_20px_rgba(48,84,255,0.2)] z-10">
                      ...
                      >
                        <step.icon
                          className="w-5 h-5 group-hover:text-white group-hover:scale-110 transition-all duration-300"
                          strokeWidth={1.5}
                        />
                      </motion.div>
                      {/* Pulse effect */}
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-white/50 leading-tight mt-1.5 group-hover:text-white/70 transition-colors">
                        {step.desc}
                      </p>
                    </div>
                  </motion.div>
            ...
            >
              ...
              <div className="relative bg-card/40 backdrop-blur-xl border border-white/5 rounded-2xl p-8 md:p-10 text-center shadow-2xl group-hover:border-[var(--brand-blue)]/30 group-hover:shadow-[0_0_40px_rgba(48,84,255,0.15)] transition-all duration-500 bg-gradient-to-br from-white/[0.04] to-transparent overflow-hidden">
                ...
                <Network
                  className="w-8 h-8 text-[var(--brand-blue)] group-hover:text-white mx-auto mb-4 opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300 relative z-10"
                  strokeWidth={1.5}
                />
                <h3 className="text-xl font-medium text-white mb-3 tracking-tight relative z-10">
                ...
                <p className="text-sm text-white/60 group-hover:text-white/80 transition-colors leading-relaxed max-w-lg mx-auto relative z-10">
                  Speech, Vision and Natural Language models collaborate through a neural fusion
                  engine to generate an objective, bias-free interview assessment.
                </p>
              </div>
            </motion.div>
          </div>
